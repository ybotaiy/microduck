...
The earlier project centered on an external routine selector above an execution state machine and the robot's existing balance controller. `DESIGN.md` v0.2 proposed measuring a command envelope, authoring six expressive routines, and comparing random, rules, and LLM selectors. Several decisions were approved that day; its opening draft status was not kept in sync with the later decision table. The current simulation-first scope above takes precedence.
...
## Historical proposed next steps — September 5 (superseded for immediate work)

The following Character Bench sequence is retained for reference. The September 9 scope above is authorized and is the current next action; these older milestones do not gate it.
...
- **September 9, 2026:** Fresh clone of the personal repository on `main` at `e72b176`; inspected the latest project documents and official CPU runner at `53b8971`. Recorded the revised push-then-ball-approach scope. No simulation, training, dependency installation, or hardware test was run during bootstrap.
